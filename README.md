# 안전한 수면을 가능케하는, 농인을 위한 위험 알림 진동 서비스
  ...
  그 어느 때보다 편안해야 하는 밤, 잠든 농인이 어떤 위험에 처해있는지 생각해보셨나요?
  저희는 그런 위험을 해소할 수 있는 '위기에서 나오는 바이브(Vibration)'를 기획했습니다.
...
## 프로젝트 추진 배경
  ...
  * 농인이란 '듣는 것보다 보는 것에 더 집중하는 존재'를 표현하는 문화적 개념입니다.
  ...
  (참고 영상 : https://youtu.be/KOu6i0Fmc9Q)</code></pre>


## 어떻게 푸는가?
  컴퓨터는 지속적으로 아기 울음소리와 사이렌 소리를 학습합니다. 구글에 'baby cry sound dataset' 등을 검색하면 나오는 풍부한 자원을 활용합니다. 

  음성 센서(LM393 등)를 이용해서 소리를 수집한 후 POST해서 서버에 데이터를 전송합니다. 서버는 Node JS로 구성되어 있고, POST 신호를 받으면 MySql에 정보를 넣습니다. 형식은 JSON 입니다.

  서버는 데이터 흐름을 파악하다가 짧은 시간의 음성 데이터가 '아기 울음소리' 및 '사이렌 소리'와 동일하다면 침대에 부착된 진동 센서 모듈에 신호를 보내서 진동하게 합니다. 잠을 자던 사람은 이 진동으로 잠에서 깨어나서 위기 상황에 대처할 수 있습니다. 
...
## 기대성과
  ...
  사회적 취약자에 해당하는 농인들의 일상 속 불편함을 해소함으로써 약자에 대한 불평등 문제 해결에 기여한다.
